...
for ring in data.get("rings", []):
    ...
    if res.returncode == 0 and os.path.exists(out_file) and os.path.getsize(out_file) > 1000:
        print(f"SUCCESS: {ring_id} -> {out_file} ({os.path.getsize(out_file)} bytes)")
        ring["imageUrl"] = f"./images/rings/{ring_id}{ext}"
    else:
        print(f"FALLBACK SVG: {ring_id} -> ./images/rings/{ring_id}.svg")
        ring["imageUrl"] = f"./images/rings/{ring_id}.svg"

with open(DATA_PATH, "w", encoding="utf-8") as f:
    json.dump(data, f, ensure_ascii=False, indent=2)

print("Finished processing all ring image downloads.")
